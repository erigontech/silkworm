...
void silkworm::read_genesis_data(unsigned int chain_id, std::string& data) {
    switch (chain_id) {
        case 1:
            assert(sizeof_genesis_mainnet_data() != 0);
            data.assign(genesis_mainnet_data(), sizeof_genesis_mainnet_data());
            break;
        case 4:
            assert(sizeof_genesis_rinkeby_data() != 0);
            data.assign(genesis_rinkeby_data(), sizeof_genesis_rinkeby_data());
            break;
        case 5:
            assert(sizeof_genesis_goerli_data() != 0);
            data.assign(genesis_goerli_data(), sizeof_genesis_goerli_data());
            break;
        default:
            // TODO Configs for ETC and Ropsten
            throw std::runtime_error("Unknown chain_id " + std::to_string(chain_id).append(" in ") +
