...
class BackEndMock : public ethbackend::BackEnd {  // NOLINT
  public:
    ...
    MOCK_METHOD((Task<bool>), get_block, (BlockNum block_num, const HashAsSpan& hash, bool, silkworm::Block&));
    MOCK_METHOD((Task<std::optional<BlockNum>>), get_block_num_from_txn_hash, (const HashAsSpan& hash));
    MOCK_METHOD((Task<std::optional<BlockNum>>), get_block_num_from_hash, (const HashAsSpan& hash));
    MOCK_METHOD((Task<std::optional<evmc::bytes32>>), get_block_hash_from_block_num, (BlockNum number));
