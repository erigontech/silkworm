...
TEST_CASE("Decode account from storage") {
    SECTION("Correct payload") {
        ...
        auto decoded{Account::from_encoded_storage(encoded)};
        REQUIRE(decoded);
    ...
    SECTION("Correct payload only incarnation") {
        Bytes encoded{*from_hex("0f01020203e8010520f1885eda54b7a053318cd41e2093220dab15d65381b1157a3633a83bfd5c9239")};
        auto incarnation{Account::incarnation_from_encoded_storage(encoded)};
        CHECK(incarnation == 5);
    }
    ...
    SECTION("One zero byte payload") {
        ...
        CHECK(Account::from_encoded_storage(encoded));
    }

    SECTION("One non-zero byte payload") {
        Bytes encoded{*from_hex("04")};
        CHECK(Account::from_encoded_storage(encoded) == tl::unexpected{DecodingError::kInputTooShort});
    }

    SECTION("One >15 byte head plus 1byte") {
        Bytes encoded{*from_hex("1e01")};
        CHECK(Account::from_encoded_storage(encoded) == tl::unexpected{DecodingError::kInvalidFieldset});
    }
    ...
    SECTION("Wrong nonce payload") {
        ...
        CHECK(Account::from_encoded_storage(encoded) == tl::unexpected{DecodingError::kLeadingZero});
    }
    ...
    SECTION("Wrong code_hash payload") {
        ...
        CHECK(Account::from_encoded_storage(encoded) == tl::unexpected{DecodingError::kUnexpectedLength});
    }
}
