...
boost::asio::awaitable<void> OtsRpcApi::handle_ots_traceTransaction(const nlohmann::json& request, nlohmann::json& reply) {
    const auto& params = request["params"];
    if (params.size() != 1) {
    ...
    try {
        ...
        const auto result = co_await executor.trace_ots_transaction(transaction_with_block.value());

        reply = make_json_content(request["id"], result.entries);
