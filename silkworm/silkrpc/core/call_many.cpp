...
awaitable<CallManyResult> CallExecutor::executes_all_bundles(const silkworm::ChainConfig* config,
    ...
    const auto execution_result = co_await boost::asio::async_compose<decltype(boost::asio::use_awaitable), void(CallManyResult)>(
        [&](auto&& self) {
            boost::asio::post(
                workers_, [&, self = std::move(self)]() mutable {
...
boost::asio::awaitable<CallManyResult> CallExecutor::execute(const Bundles& bundles, const SimulationContext& context,
    ...
    co_return co_await executes_all_bundles(chain_config_ptr, *block_with_hash, tx_database, bundles, opt_timeout, accounts_overrides, transaction_index);
}
