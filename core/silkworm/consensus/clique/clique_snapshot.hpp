...
class CliqueSnapshot {
  public:
    ...
    CliqueSnapshot(uint64_t block_number, evmc::bytes32 hash, std::vector<evmc::address> signers,
        ...
        : block_number_{block_number}, hash_{hash}, signers_{std::move(signers)}, recents_{std::move(recents)} {}
    ...
    ValidationResult verify_seal(const BlockHeader& header, const evmc::address& signer) const;
