...
class BlockCacheNEW {
  public:
    explicit BlockCacheNEW(std::size_t capacity = 1024, bool shared_cache = true)
        : block_cache_(capacity), shared_cache_(shared_cache) {}

    boost::optional<std::shared_ptr<silkworm::BlockWithHash>> get(const evmc::bytes32& key) {
        if (shared_cache_) {
  ...
  private:
    ...
    boost::compute::detail::lru_cache<evmc::bytes32, std::shared_ptr<silkworm::BlockWithHash>> block_cache_;
    bool shared_cache_;
};
