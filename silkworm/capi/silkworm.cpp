...
static log::Args log_args_for_exec_progress(ExecutionProgress& progress, uint64_t current_block) {
    ...
    batch_progress_perc << std::fixed << std::setprecision(2) << progress.batch_state_perc * 100 << "%";
    return {
        "number",
        std::to_string(current_block),
        "blk/s",
    ...
    try {
        ...
        while (block_number <= max_block) {
            while (block_number <= max_block && state_buffer.current_batch_state_size() < max_batch_size) {
                ...
                state_buffer.begin_block(block->header.number);
                ...
                if (log_time <= now) {
                    progress.batch_state_perc = float(state_buffer.current_batch_state_size()) / float(max_batch_size);
