...
void to_json(nlohmann::json& json, const Block& b) {
    ...
    if (b.full_tx) {
        ...
        for (std::size_t i{0}; i < json["transactions"].size(); i++) {
            ...
            json_txn["gasPrice"] = to_quantity(b.block_with_hash->block.transactions[i].effective_gas_price(b.block_with_hash->block.header.base_fee_per_gas.value_or(0)));
        }
    } else {
        ...
        for (std::size_t i{0}; i < b.block_with_hash->block.transactions.size(); i++) {
            auto ethash_hash{hash_of_transaction(b.block_with_hash->block.transactions[i])};
