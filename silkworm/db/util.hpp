...
inline mdbx::slice to_slice(const evmc::bytes32& value) {
    return sw_mdbx::to_slice(ByteView{value.bytes});
}

inline mdbx::slice to_slice(const evmc::address& address) {
    return sw_mdbx::to_slice(ByteView{address.bytes});
}
...
std::optional<ByteView> find_value_suffix(sw_mdbx::ROCursorDupSort& table, ByteView key, ByteView value_prefix);

// We can't simply call upsert for storage values because they live in mdbx::value_mode::multi tables
void upsert_storage_value(sw_mdbx::RWCursorDupSort& state_cursor, ByteView storage_prefix, ByteView location, ByteView new_value);
