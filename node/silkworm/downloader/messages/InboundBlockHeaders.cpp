...
void InboundBlockHeaders::execute() {
    ...
    */
    log::WarningChannel() << "Handling of " << identify(*this) << " is incomplete\n";

    if (penalty != Penalty::NoPenalty) {
        log::TraceChannel() << "Replying to " << identify(*this) << " with penalize_peer\n";
        rpc::PenalizePeer penalize_peer(peerId_, penalty);
        sentry_.exec_remotely(penalize_peer);
    }

    log::TraceChannel() << "Replying to " << identify(*this) << " with peer_min_block\n";
