...
bool Decompressor::read_ahead(uint64_t start_offset, uint64_t end_offset, ReadAheadFuncRef fn) {
    ensure(bool(compressed_file_), "decompressor closed, call open first");
    compressed_file_->advise_sequential();
    [[maybe_unused]] auto _ = gsl::finally([&]() { compressed_file_->advise_random(); });
    Iterator it{this, start_offset, end_offset};
    return fn(it);
}
...
Decompressor::Iterator::Iterator(const Decompressor* decoder, uint64_t start_offset, uint64_t end_offset) : decoder_(decoder), word_offset_{start_offset}, end_offset_{end_offset} {}
