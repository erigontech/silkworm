...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        MDB_val mdb_data;
        SILKWORM_LOG(LogLevels::LogInfo) << "Started Tx Lookup Extraction" << std::endl;
        ...
        while (!rc) {                                     /* Loop as long as we have no errors*/
            ...
            if (block_number % 100000 == 0) {
                SILKWORM_LOG(LogLevels::LogInfo) << "Tx Lookup Extraction Progress << " << block_number << std::endl;
            }
        ...
        SILKWORM_LOG(LogLevels::LogInfo) << "Entries Collected << " << collector.size() << std::endl;

        // Proceed only if we've done something
        if (collector.size()) {
            SILKWORM_LOG(LogLevels::LogInfo) << "Started tx Hashes Loading" << std::endl;
