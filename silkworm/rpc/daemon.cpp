...
std::unique_ptr<db::kv::api::Client> Daemon::make_kv_client(rpc::ClientContext& context) {
    ...
    if (settings_.standalone) {
        return std::make_unique<db::kv::grpc::client::RemoteClient>(
            create_channel_, grpc_context, state_cache, block_provider(backend), block_number_from_txn_hash_provider(backend), block_number_from_block_hash_provider(backend), block_hash_from_block_number_provider(backend));
    }
