...
TEST_CASE("Legacy Transaction RLP") {
    ...
    REQUIRE(!error(rlp::decode<Transaction>(view, decoded)));
    CHECK(view.empty());
    CHECK(decoded == txn);
...
TEST_CASE("EIP-2930 Transaction RLP") {
    ...
    REQUIRE(!error(rlp::decode_transaction(view, decoded, rlp::Eip2718Wrapping::kNone)));
    CHECK(view.empty());
    CHECK(decoded == txn);
    ...
    view = encoded_raw;
    REQUIRE(!error(rlp::decode_transaction(view, decoded, rlp::Eip2718Wrapping::kBoth)));
    CHECK(view.empty());
    CHECK(decoded == txn);
    ...
    view = encoded_wrapped;
    REQUIRE(!error(rlp::decode_transaction(view, decoded, rlp::Eip2718Wrapping::kString)));
    CHECK(view.empty());
    CHECK(decoded == txn);
