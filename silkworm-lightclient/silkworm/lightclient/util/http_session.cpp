...
// TODO(canepat) replace w/ boost.url (boost 1.81+)
#include <optional>
#include <regex>
...
inline static url parse_url(const std::string& uri) {
    static const std::regex kUrlRegex{
        R"((([httpsw]{2,5})://)?([^/ :]+)(:(\d+))?(/([^ ?]+)?)?/?\??([^/ ]+\=[^/ ]+)?)",
        std::regex_constants::ECMAScript | std::regex_constants::icase};
    std::smatch match;
    url parsed_url;
    if (std::regex_match(uri, match, kUrlRegex) && match.size() == 9) {
        parsed_url.protocol = match[2].str().empty() ? "http" : match[2].str();
        parsed_url.host = match[3].str();
        parsed_url.port = match[5].str().empty() ? (parsed_url.protocol == "https" ? "443" : "80") : match[5].str();
        parsed_url.target = match[6].str().empty() ? "/" : match[6].str();
        parsed_url.query = match[8];
    }
    return parsed_url;
}
...
// TODO(canepat) replace w/ real TLS certificate verification
...
net::awaitable<Bytes> do_http_session(const std::string& uri) {
    ...
    http::parser<false, http::string_body> response_parser{std::move(response)};
    response_parser.eager(true);
    response_parser.body_limit(boost::none);

    // Receive the HTTP response
    co_await http::async_read(ssl_stream, b, response_parser, net::use_awaitable);
    ...
    const auto& response_body = response_parser.get().body();
    co_return Bytes{response_body.cbegin(), response_body.cend()};
}
