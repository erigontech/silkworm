...
int do_compact(db_options_t& db_opts, compact_options_t& app_opts) {
    ...
    try {
        ...
        if (!src_nosubdir) src_path /= fs::path{db::kDb_data_file_name};
        ...
        if (!src_nosubdir) tgt_path /= fs::path{db::kDb_data_file_name};
        ...
        if (!shouldStop) {
            ...
            if (app_opts.replace && !src_nosubdir) {
                ...
                if (!app_opts.nobak) {
                    ...
                    std::string src_file_back{db::kDb_data_file_name};
...
int main(int argc, char* argv[]) {
    ...
    if (app_compact) {
        ...
        compact_opts.file = (compact_opts.dir / fs::path(db::kDb_data_file_name));
        if (fs::exists(compact_opts.file)) {
            std::cout << " An " << db::kDb_data_file_name << " file already present in workdir" << std::endl;
            return -1;
        }
    }
    ...
    if (app_copy) {
        ...
        copy_opts.file = (copy_opts.dir / fs::path(db::kDb_data_file_name));
        if (fs::exists(copy_opts.file)) {
            copy_opts.filesize = fs::file_size(copy_opts.file);
            if (copy_opts.create) {
                std::cout << db::kDb_data_file_name
                          << " file already present in target directory but you have set --create" << std::endl;
                return -1;
            }
        } else if (!copy_opts.create) {
            std::cout << db::kDb_data_file_name << " not found in target directory. You may want to specify --create"
                      << std::endl;
            return -1;
        }
