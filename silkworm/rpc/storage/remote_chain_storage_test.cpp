...
struct RemoteChainStorageTest : public test::ContextTestBase {
...
TEST_CASE_METHOD(RemoteChainStorageTest, "read_chain_config") {
    SECTION("empty chain data") {
        EXPECT_CALL(transaction, get_one(db::table::kCanonicalHashesName, _)).WillOnce(InvokeWithoutArgs([]() -> Task<silkworm::Bytes> {
            co_return kBlockHash;
        }));
        EXPECT_CALL(transaction, get_one(db::table::kConfigName, _)).WillOnce(InvokeWithoutArgs([]() -> Task<silkworm::Bytes> {
