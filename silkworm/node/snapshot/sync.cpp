...
static std::string dump_all_torrent_counters(const std::vector<lt::stats_metric>& metrics, lt::span<const int64_t> counters) {
    std::string counters_dump;
    for (int i{0}; i < counters.size(); ++i) {
        const auto& stats_metric = metrics.at(static_cast<std::size_t>(i));
        counters_dump.append(stats_metric.name);
        counters_dump.append("=");
