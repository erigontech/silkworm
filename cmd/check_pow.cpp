...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        log::InfoChannel() << "Initializing Light Cache for DAG epoch " << epoch_num;
        ...
        for (uint32_t block_num{options.block_from}; block_num <= options.block_to && !SignalHandler::signalled();
            ...
            if (!(block_num % 1000)) {
                ...
                log::InfoChannel() << "At block height " << block_num << " in " << sw.format(interval.second);
            }
        }

        log::InfoChannel() << "Complete !";

    } catch (const fs::filesystem_error& ex) {
        log::ErrorChannel() << ex.what() << " Check your filesystem permissions";
