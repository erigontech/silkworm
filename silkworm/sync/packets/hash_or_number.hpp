...
namespace rlp {
    ...
    inline DecodingResult decode(ByteView& from, HashOrNumber& to, bool allow_leftover = false) noexcept {
        ...
        if (h->payload_length == 32) {
            Hash hash;
            if (DecodingResult res = rlp::decode(from, static_cast<evmc::bytes32&>(hash), allow_leftover); !res) {
                return res;
            }
            to = {hash};
        } else if (h->payload_length <= 8) {
            BlockNum number{};
            if (DecodingResult res = rlp::decode(from, number, allow_leftover); !res) {
                return res;
            }
            to = {number};
        } else {
