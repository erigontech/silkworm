...
template <>
struct adl_serializer<silkworm::BlockNumOrHash> {
    static silkworm::BlockNumOrHash from_json(const json& json) {
        if (json.is_string()) {
            return silkworm::BlockNumOrHash{json.get<std::string>()};
        }
        if (json.is_number()) {
            return silkworm::BlockNumOrHash{json.get<silkworm::BlockNum>()};
        }
        return silkworm::BlockNumOrHash{0};
    }
};
